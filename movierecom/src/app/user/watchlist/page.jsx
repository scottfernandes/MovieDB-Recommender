 ...
 const username=session.user.email
    ...
    const removeMovie = async (movieName, moviePoster) => {
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          setMovies(prevMovies => prevMovies.filter(movie => movie.title !== movieName));
  ...
  <div className="head">
    ...
    <div className="watchlist mt-10 text-white px-4 sm:px-8 md:px-12">
      <p className={`${lancelot.className} text-4xl sm:text-5xl md:text-6xl lg:text-7xl text-center`}>
        My Watchlist
      </p>
      ...
      <div className="mt-12 sm:mt-16 md:mt-20 lg:mt-28">
        {movies.length>0?(movies.map((movie, index) => (
            ...
            <div className="det md:ml-12 lg:ml-16 mt-6 md:mt-16 text-center md:text-left">
              ...
              <div className="btns mt-6 md:mt-16 flex flex-col md:flex-row items-center justify-center md:justify-start gap-4">
                <Button
                  ...
                  onClick={() => router.push(`/user/recom/${encodeURIComponent(movie.title)}`)}
                >
                  View Details
                </Button>
                <Button
          ...
          <AlertDescription className='text-2xl ml-5 '>
            {error}
          </AlertDescription>
          </div>
        </Alert>)}
